...
// Interface for style creation options
interface _StyleCreationOptions {
	colorName: string;
	organizationStyle: "flat" | "nested";
	createVariables: boolean;
	createStyles: boolean;
	variableMode?: "local" | "collection";
}
...
function generateColorRamp(
	...
	const CONTRAST_TARGETS: Record<string, number> = {
		"100": 12,
		"200": 8,
		"300": 5.5,
		"400": 4.5,
	...
	// Get chroma multipliers from the flavor
	const chromaMultipliers = selectedFlavor.chromaMultipliers;
...
function createColorRampFrames(seedColor: string, flavor: string = "bright") {
	// Generate the color ramp
	const colorRamp = generateColorRamp(seedColor, flavor);
	const rampEntries = Object.entries(colorRamp);

	// Create a parent frame to contain all color frames
	const parentFrame = figma.createFrame();
	parentFrame.name = `[Ramp] ${
...
figma.ui.onmessage = async (msg: UiMessage) => {
	if (msg.type === "create-ramp") {
		try {
			// Create color ramp frames with specified flavor
			createColorRampFrames(msg.color || "#000000", msg.flavor || "bright");
